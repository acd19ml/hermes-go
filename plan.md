...
| Phase 1 ✅ | 单轮 LLM 调用 | Provider Transport | 先具体 OpenAI，再提最小 client seam |
...
**Phase 0 确立的设计决策（后续阶段遵守）：**

- **内部格式 = OpenAI-style**：`Message`、`ToolCall`、`ToolResult` 均对齐 OpenAI Chat Completions 语义。各 provider-specific wire format 的转换由对应 client 负责（Phase 1 是 `OpenAIChatClient`，Phase 6 加入 `AnthropicClient`），Phase 6 的 Transport 再泛化提取。
- `ToolCall.Arguments string`（JSON string，对应 OpenAI `function.arguments`），不用 `json.RawMessage`。
- `ToolResult` 含 `Name string`（工具名，OpenAI wire 要求，也用于 session DB）。
- `StaticResponder.Respond(ctx context.Context, msgs []Message) (Message, error)` 签名接收完整消息列表（含 system），忽略 system message，只回显最后一条 user message；Phase 1 c2 提取 `chatClient` interface 时两者签名天然对齐。

### Phase 1: 第一次真实 LLM，无工具 ✅

目标：先把真实 OpenAI Chat Completions API 跑通，再抽最小 seam 给 Agent。内部格式已是 OpenAI-style，client 层几乎零转换。

**实现方式：1 个 issue（#4），5 个有序 commit（不拆 5 个独立 PR）。**

| Commit | 内容 | 验收 | 抽象纪律 |
|---|---|---|---|
| c1 ✅ | `OpenAIChatClient`：内部消息 → OpenAI Chat Completions wire，只支持 user/assistant text；补 `RoleSystem` 常量 | 无 key 时返回 error；httptest 验证 success/non-200 | 不提 Transport；转换逻辑住在 client 里 |
| c2 ✅ | `AIAgent.RunOnce(ctx, userMsg)`；提未导出 `chatClient interface { Respond(ctx, []Message) }`；`StaticResponder` 签名升级 | Agent 单元测试覆盖 success/error；`--msg` 切换到 AIAgent | Static + OpenAI 两个实现，现在合法提 interface |
| c3 ✅ | 硬编码 system prompt，以 `Message{Role:RoleSystem}` 预置在消息列表首位 | system prompt 内容正确；byte-static 测试 | 不拆 PromptBuilder；system message 对 OpenAI wire 直接透传 |
| c4 ✅ | `IterationBudget` 数据结构，loop 仍只跑一轮 | budget exhausted 有测试，验证 LLM 不被调用 | 不做自动继续 |
| c5 ✅ | `--configure` 向导 + `internal/config`：交互式填写 Base URL / API Key，`GET /models` 验证，分页选模型，保存 `~/.hermes-go/config.json`；`NewOpenAIChatClientFromEnv` 降级链 env → config → 默认值 | `--configure` 后无 env var 可用 `--msg`；`HERMES_CONFIG=/dev/null` 测试隔离 | 不引入第三方库；不做 GUI |

**Wire format 职责划分（Phase 1 确立，Phase 6 泛化）：**

```
Go 内部 (OpenAI-style)          OpenAIChatClient               OpenAI wire
─────────────────────           ─────────────────              ────────────
Message{Role:"system"}    ──►  直接映射                  ──►  {"role":"system","content":"..."}
Message{Role:"user"}      ──►  直接映射                  ──►  {"role":"user","content":"..."}
Message{Role:"assistant"} ◄──  choices[0].message 提取   ◄──  {"choices":[{"message":{...}}]}
```

内部格式 = OpenAI wire（Phase 0 确立），Phase 1 的 `OpenAIChatClient` 转换代价极低。
Phase 6 加入 `AnthropicClient` 作为第二条 API 路径，届时才需要做 OpenAI→Anthropic wire 转换，再提 Transport 泛化。

Wiki 对齐点：Hermes 的 `run_conversation` 是同步循环，内部消息遵循 OpenAI role 形状（已由 Phase 0 确立）；Go 版此阶段只复现一轮。
...
| PR | 内容 | 验收 | 不做 |
|---|---|---|---|
| 2.1 | OpenAI tool-call wire parsing：`tool_calls[].function` → 内部 `ToolCall{ID,Name,Arguments}` | fixture 测试解析 | 不执行工具 |
...
目标：Phase 1 已有 `OpenAIChatClient`（OpenAI-style）。Phase 6 加入 `AnthropicClient` 作为第二条 API 路径（需要做 OpenAI-style → Anthropic wire 的非平凡转换），两条路径都存在后再提 Transport 泛化。

| PR | 内容 | 验收 | 抽象纪律 |
|---|---|---|---|
| 6.1 | `AnthropicClient`：OpenAI-style 内部消息 → Anthropic Messages API wire（system 提取、content block 转换、tool_use/tool_result 映射） | fixture + 可选真实调用 | 暂时重复转换代码；不提 Transport |
| 6.2 | 对比 OpenAI/Anthropic 差异，重构 `Transport`：ConvertMessages/ConvertTools/BuildRequest/Normalize | 两个 client 行为不变 | 现在才有两条 API mode |
| 6.3 | `NormalizedResponse`：text/tool_calls/usage/finish_reason | 两 provider fixture 归一 | Agent loop 只看 normalized |
| 6.4 | provider-specific quirks：Anthropic system 字段、OpenAI developer role 切换 | fixture 测试 | 不散落到 agent loop |
| 6.5 | `ProviderProfile` 数据类：auth/env/base_url/model defaults | 第二个 OpenAI-compatible provider 出现后再落地 | profile 纯数据，不持 client |
...
> **内部格式锚定（Phase 0 确立）**：Go 内部统一使用 OpenAI-style。
> Provider wire format 的转换由各 client 负责；Phase 6 Transport 泛化提取。

| Hermes/Python | Go 映射 | 禁忌 |
|---|---|---|
| dict message | `struct Message { Role, Content, ToolCalls, ToolCallID }` | 不用 `map[string]any` 贯穿业务层 |
| `{“role”:”system”,”content”:”...”}` | `Message{Role: RoleSystem, Content: “...”}` 作为消息列表首条 | 不单独开 system 参数传给 interface；client 负责提取 |
| tool `input` object (Anthropic) / `arguments` string (OpenAI) | `ToolCall.Arguments string`（JSON string，OpenAI 语义） | 不用 `json.RawMessage`；转 Anthropic wire 时 `json.Unmarshal` |
| tool result `name` field | `ToolResult.Name string` | 不省略；OpenAI wire 要求，session DB 也需要 |
| `chatClient` / provider seam | 未导出 `chatClient interface { Respond(ctx context.Context, msgs []Message) (Message, error) }`；msgs 含完整历史（含 system） | 不在单一实现时提；Static + OpenAI 两个后才出现（Phase 1 c2）|
